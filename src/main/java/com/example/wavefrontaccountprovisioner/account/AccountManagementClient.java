...
public class AccountManagementClient {
     ...
     *                            be determined.
     */
    public AccountManagementClient(RestTemplateBuilder restTemplateBuilder, @Nullable String version, Duration timeout) {
        this.restTemplate = restTemplateBuilder.setConnectTimeout(timeout).setReadTimeout(timeout).build();
    ...
    private URI accountManagementUri(String clusterUri, ApplicationTags applicationTags) {
        UriComponentsBuilder uriComponentsBuilder = UriComponentsBuilder.fromUriString(clusterUri)
                ...
                .queryParam("application", applicationTags.application())
                .queryParam("service", applicationTags.service());
        if (applicationTags.cluster() != null) {
            uriComponentsBuilder.queryParam("cluster", applicationTags.cluster());
        }
        if (applicationTags.shard() != null) {
            uriComponentsBuilder.queryParam("shard", applicationTags.shard());
        }
